...
An *enumeration*, or *enum*, defines a finite set of values and enables you to work with those values in a type-safe way within your code. An enum is declared with the `enum` keyword.
```swift
enum Direction {
...
```
In most cases, the enum type can be inferred, allowing you to use the dot syntax
```swift
currDirection = .south
```
To check the case of an enum, you can use a `switch`
```swift
...
```
The `if-case` statement works similarly to the `if-let` statement we saw in Lesson 0: both can be combined together or with other boolean predicates using `,`.
...
```
However, the example above might seem a bit trivial, because the real power of protocols is when we want to define multiple entities.
```swift
...
```
Then imagine that in CalCentral, we want to have a list that stores all the users, whether they are `Student` or `DeNero`. We can just say
```swift
class CalCentral {
...
A closure is an anonymous function that can be passed around and used in your code. It is similar in the way how lambda expression works in Python, only more powerful. You do not need any special keyword to declare a closure, you simply define it using a set of curly brackets `{ }`
```swift
{ print("This is a closure") }
```
It can be called just like any normal function with parentheses `()`, as you've already seen in our UI components declaration
```swift
{
    let label = UILabel()
    /* Customization */
...
```
A closure can have one or more parameters, defined at the beginning of the closure, and are separated from the body of the closure using a keyword `in`
```swift
{ (foo: String, bar: Int) in
    print("foo: \(foo), bar: \(bar)")
}
```
A closure can be assigned as a variable, whose type will then have to match the parameter types as well as the return type of the assigned closure.
```swift
let myClosure: (Int)->String = { (number: Int) in
...
```swift
let myClosure: (Int)->String = { number in
    return String(number)
}
```
...
```swift
let myClosure: (Int)->String = { // Error
    return String(number)
}

let myClosure: (Int)->String = { (number: Float) in // Error
    return String(number)
}

let myClosure: (Int)->String = { number, something in // Error
    return String(number)
}
```
...
If the body of a closure only has a single statement, the *return* keyword is inferred
```swift
let myClosure: (Int)->String = { number in
    String(number)
}
...
In the case above, the argument list and the `in` keyword can also be omitted.

```swift
let myClosure: (Int)->String = { String($0) }
```

# Collection View

So far we've seen quite a few basic components from UIKit: `UILabel` for text, `UIButton` for interacting with the user, `UIImageView` (from project 1) for displaying an image, and `UIStackView` (from demo 1) for easier layout. 
...
While there're many more built-in components like those shown in the picture above, for now we will shift our focus towards the compositional side the UI, meaning how to combine these basic components into a coherent user experience.

For iOS, one of the most powerful tools in our repertoire is the `UICollectionView`. It is a view that manages and presents an ordered, structured collection of data items. Collection views have come a long way since it was first introduced in iOS 6. In WWDC19, it received another major update which makes it extremely versatile. 

<img src="/assets/images/appstore-collection-view.jpeg"
    alt="Collection View in iOS"
    style="width: 50%; display: block; margin: auto;" />

This entire page of the App Store is implemented with a single collection view. Although we won't go into these latest updates in this lesson, we might revisit at a later date if we have time. For those who are already familiar with `UICollectionView` and interested in this topic, check out this [WWDC19 session](https://developer.apple.com/wwdc19/215)

## Instantiating a UICollectionView

The most basic collection view comprises a layout object and some cells, and the most basic type of layout you can use is the flow layout. It is shipped with the concrete `UICollectionViewFlowLayout` class and is available in UIKit.
```swift
...
<div style="width: 90%; margin: auto;">
<iframe width="90%" height="315" src="https://www.youtube.com/embed/2WQzAWhvGtk" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>
